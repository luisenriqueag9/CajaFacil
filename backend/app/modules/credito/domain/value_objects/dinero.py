...
class Dinero:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.divisa, str) or len(self.divisa) != 3:
            raise CreditoInvalidoException("La divisa debe ser un codigo ISO de 3 caracteres.")
        ...
        if isinstance(self.monto, (int, float)):
            ...
            raise CreditoInvalidoException("El monto debe ser un numero decimal, entero o flotante.")
    ...
    def sumar(self, otro: "Dinero") -> "Dinero":
        if self.divisa != otro.divisa:
            raise CreditoInvalidoException(f"No se pueden sumar montos con diferentes divisas: {self.divisa} y {otro.divisa}.")
        return Dinero(self.monto + otro.monto, self.divisa)

    def restar(self, otro: "Dinero") -> "Dinero":
        if self.divisa != otro.divisa:
            raise CreditoInvalidoException(f"No se pueden restar montos con diferentes divisas: {self.divisa} y {otro.divisa}.")
        return Dinero(self.monto - otro.monto, self.divisa)
    ...
    def __lt__(self, otro: "Dinero") -> bool:
        if self.divisa != otro.divisa:
            raise CreditoInvalidoException("No se pueden comparar montos con diferentes divisas.")
